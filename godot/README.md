...
独立 3D 迁移试玩：运行仓库根目录 `启动3D迁移验证.cmd`。左上“构图对照”可查看行进／战斗的传统版与 3D 静态采集图，打开时暂停、关闭时恢复此前暂停状态。图片来自 `tempassets/work/`，显示文件时间；不是实时同步画面，编辑后需重新采集。原版和保存的编辑数据继续保留。缺少本机截图时面板会提示未采集，不影响试玩。

事件“准备迎战”和验证面板“普通遭遇”使用独立波次配置 `data/world3d_encounter_wave.json`（试玩初值 24 敌人、活跃上限 14），陆续生成并保留原先出现的怪物。“50 来敌”保留原压力入口。普通遭遇的数量和强度仍待正式玩法平衡，旧版配置不变。

左上工具栏新增“二岔测试”“三岔测试”：重新准备当前主题，从路口前约三秒开始，选择方向后实际转弯前进，不插入战斗。转弯结束可接着测试另一种岔路，或用顶部按钮重复当前类型；“普通流程”退出此测试并重新开始普通流程。切换有加载过程，不是连续地图中的无缝切换，不改写存档。加载期间遮住尚未初始化的场景，镜头与角色站位就绪后才显示。

**当前默认入口是雾林开始界面**（`scenes/mistwood_start.tscn`）。家园「灯下寓所」支持：点击地块种植、搬迁、解锁，收获后安排出征；冒险岛入口进入森林/洞穴/云海局内路线，再处理怪物或旅人事件，携行囊返航。此前三战换岛是历史联调方案。当前结构和限制见[主岛与出征设计](../docs/design/HOME-AND-EXPEDITIONS.md)。通过主岛右上「表现样本」仍可进入原对照目录。

默认运行打开雾林开始界面。`./godot/run.ps1 -Battle` 直接进入独立战斗样本；`-World` 进入两种浮岛；`-Spaces` 进入连续空间；`-Forest` 进入原森林 A/B。森林场景可用 Esc 返回目录。
...
战斗中点击左侧卡牌加入，右键移除，拖动己方卡牌换位，双击封印物切换手持/操控。可先看“默认阵容”，再试“护阵示例”。支持暂停、0.5/1/2/4 倍速、换敌与第 1–32 关参数。试验库直接提供 24 张原卡牌，不代表已接入捕获、装备或天赋成长。
...
Godot 默认使用黑暗童话内容。已删除旧境界属性加成、替换原生卡牌编号与文案，并移除旧卡面及其导入文件。网页 demo 的代码和资产保留。旧名称仅存在于 `native_save_upgrade.gd` 的兼容映射中，用于导入旧用户目录和迁移阵容、日志及编辑器设置；原始存档不覆盖。
