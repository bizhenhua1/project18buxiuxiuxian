...
static func make_space(key:String) -> SpaceType:
 ...
 space.top_color=c.fog.darkened(.7);space.ground_texture=load(FairytaleCatalog.asset(key,"ground.png") if FairytaleCatalog.has_scene(key) else "res://assets/biomes/%s/ground.png"%key)
