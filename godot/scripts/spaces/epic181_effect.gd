...
func advance(dt:float):
 ...
 for layer in layers:
  ...
  if not stopped and time>=0 and (d.loop or time<d.duration):
   ...
   var limit:int=int(layer.get("instance_particle_limit",d.get("runtime_particle_limit",CAP)))
   var accepted:=mini(count,maxi(0,limit-layer.particles.size()))
   if limit<CAP:budget_skipped_spawns+=count-accepted
   var spawn_phase:float=fmod(age,maxf(.01,d.duration))/maxf(.01,d.duration)
   for i in accepted:spawn(layer,previous_position.lerp(emission_position,float(i+1)/maxi(1,count)),emitter_basis,spawn_phase)
   while layer.burst<d.bursts.size() and local_time>=d.bursts[layer.burst].time:
    if rng.randf()<=d.bursts[layer.burst].get("probability",1):
     var requested:=mini(CAP,int(sample(d.bursts[layer.burst].count,t)))
     var burst_accepted:=mini(requested,maxi(0,limit-layer.particles.size()))
     if limit<CAP:budget_skipped_spawns+=requested-burst_accepted
     for i in burst_accepted:spawn(layer,emission_position,emitter_basis,spawn_phase)
    layer.burst+=1
