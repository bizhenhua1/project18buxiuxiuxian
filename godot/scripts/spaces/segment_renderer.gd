...
func bind_combat_lights(shader:ShaderMaterial) -> void:
	var values:=combat_light_parameters()
	for key in values:shader.set_shader_parameter(key,values[key])
func combat_light_parameters() -> Dictionary:
	var values:Dictionary={}
	var team=team_light()
	values["team_light_energy"]=team.road_energy
	values["team_light_radius"]=team.road_radius
	values["team_light_color"]=team.road_color*environment_light_tint()
...
func biome_parameters() -> Dictionary:
	...
	values["enemy_scene_tint"]=enemy_light_tint()
	values["environment_light_tint"]=environment_light_tint()
	values["biome_kind"]=catalog.TITLES.keys().find(key)+1 if active else 0
	if not active:return values
	if FairytaleCatalog.has_scene(key):values["fairytale_mist_color"]=Color(FairytaleCatalog.entry(key).fog)
	...
	values["biome_lights"]=biome_light_cache
	values["biome_glow_color"]=catalog.CONFIG[key].color
