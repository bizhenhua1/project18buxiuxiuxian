# Frozen before survivor compaction and spawn-color caching; differential test only.
extends Node3D
...
func advance(dt:float):
 ...
 var emitter_basis:=global_basis
 ...
 var billboard_basis:=emitter_basis.orthonormalized().inverse()*camera_basis
 var horizontal_basis:=emitter_basis.orthonormalized().inverse()*Basis(Vector3.RIGHT,PI*.5)
 ...
 for layer in layers:
  ...
  for p in layer.particles:
   ...
   if not d.velocity.is_empty():
    ...
    if local and d.get("velocity_world",false):vel=inverse_basis*vel
    elif not local and not d.get("velocity_world",false):vel=emitter_basis*vel
    p.position+=vel*dt
   ...
   var pos:Vector3=(p.position if local else inverse_transform*p.position)+Vector3(sin(p.age*13+random*12),cos(p.age*17+random*6),sin(p.age*11))*noise
   ...
   if int(d.render_mode)!=4:
    ...
    if int(d.render_mode)==2:basis=horizontal_basis
