...
const OLD=preload("res://tests/fixtures/epic181_budget_spawn_reference.gd")
...
func run():
 ...
 for id in [entry.id,entry.muzzle_id,entry.impact_id]:
  ...
  for actor in pair:
   for i in actor.layers.size():actor.layers[i].instance_particle_limit=0 if i%2==0 else 2
  for frame in 120:
   ...
   assert(pair[0].rng.state==pair[1].rng.state)
   assert(pair[0].budget_skipped_spawns==pair[1].budget_skipped_spawns)
 ...
 print("EPIC_BUDGET_SPAWN_PASS samples=",samples," max_error=",worst," reference_usec=",timings[0]," cached_usec=",timings[1])
 quit()
