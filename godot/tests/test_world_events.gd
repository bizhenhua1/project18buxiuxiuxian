...
func run() -> void:
	...
	state.world.advance(IslandModel.STEP_SECONDS+0.001)
	assert(state.world.rebounding and state.world.player==start)
	assert(state.world.explored.has(target) and not state.pending.is_empty())
	var initial:=state.world.avatar()
	assert(Vector2(initial.x,initial.z).distance_to(Vector2(target))<0.001)
