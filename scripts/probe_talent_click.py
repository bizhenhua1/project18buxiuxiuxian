...
def main():
    ...
    try:
        ...
        time.sleep(0.5)
        print("after real click b1:", ev(ws, "(async () => { const m = await import('./js/talents.js?v=dao11'); return JSON.stringify({ alloc: [...m.allocatedIds()], b1: m.allocatedIds().has('b1') }); })()"))
        # 真实点击 b2（依赖 b1 相邻）
        ...
        time.sleep(0.3)
        print("after real click b2:", ev(ws, "(async () => { const m = await import('./js/talents.js?v=dao11'); return JSON.stringify([...m.allocatedIds()]); })()"))
        # 再点一次 b2 = 退点
        click_at(ws, rect2["x"], rect2["y"])
        time.sleep(0.3)
        print("after dealloc b2:", ev(ws, "(async () => { const m = await import('./js/talents.js?v=dao11'); return JSON.stringify([...m.allocatedIds()]); })()"))
        ...
        time.sleep(0.3)
        print("after drag-onto-node:", ev(ws, "(async () => { const m = await import('./js/talents.js?v=dao11'); return JSON.stringify([...m.allocatedIds()]); })()"))
        # 洗髓按钮
        print("respec:", ev(ws, "(async () => { document.getElementById('talent-respec').click(); const m = await import('./js/talents.js?v=dao11'); return JSON.stringify([...m.allocatedIds()]); })()"))
        ws.close()
    finally:
        proc.kill()
