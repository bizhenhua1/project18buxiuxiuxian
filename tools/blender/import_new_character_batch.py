...
for entry in json.loads((BASE/'tools/blender/new_character_batch.json').read_text(encoding='utf-8')):
 key=entry['key']
 bpy.ops.wm.read_factory_settings(use_empty=True)
 pmx=BASE/entry['source'];bpy.ops.mmd_tools.import_model(filepath=str(pmx),scale=.1)
